...
def _require_explicit_direct_db_override(command_name: str) -> None:
    # Prevent accidental direct DB writes/reads outside the API-key auth flow.
    if os.getenv("ALLOW_DIRECT_DB_CLI", "0") == "1":
        return
    typer.echo(
        f"Direct '{command_name}' CLI access is disabled. "
        "Use key-authenticated API flow via ./food-tracker with FOOD_TRACKER_API_KEY.",
        err=True,
    )
...
def add(title: str, kcal: float, fat: float, carbs: float, protein: float, 
    ...
    _require_explicit_direct_db_override("add")
...
def list(limit: int = 10):
    """List recent entries."""
    _require_explicit_direct_db_override("list")
...
def delete(entry_id: int):
    ...
    _require_explicit_direct_db_override("delete")
