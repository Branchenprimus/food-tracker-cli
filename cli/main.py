...
from config.config import DEFAULT_CONFIDENCE, DATE_FORMAT, TIME_FORMAT
...
def add(
    ...
    date: Annotated[Optional[str], typer.Option(help="DD.MM.YYYY")] = None,
    time: Annotated[Optional[str], typer.Option(help="HH:MM")] = None,
    json: bool = False
):
    """Add a new food entry."""
    entry_date = datetime.strptime(date, DATE_FORMAT).date() if date else None
    entry_time = datetime.strptime(time, TIME_FORMAT).time() if time else None
...
@app.command("list")
def list_entries(
    start: Annotated[Optional[str], typer.Option("--from", help="DD.MM.YYYY")] = None,
    end: Annotated[Optional[str], typer.Option("--to", help="DD.MM.YYYY")] = None,
    limit: int = 50,
    json: bool = False
):
    """List food entries."""
    start_date = datetime.strptime(start, DATE_FORMAT).date() if start else None
    end_date = datetime.strptime(end, DATE_FORMAT).date() if end else None
...
@app.command()
def week(
    ending: Annotated[Optional[str], typer.Option(help="DD.MM.YYYY")] = None,
    json: bool = False
):
    """Show weekly summary (last 7 days)."""
    end_date = datetime.strptime(ending, DATE_FORMAT).date() if ending else datetime.now().date()
...
def day(
    ...
    """Show daily totals."""
    day_date = datetime.strptime(date, DATE_FORMAT).date() if date else datetime.now().date()
